...
//    }
    ...
    public function save()
    {
        ...
        $form->sport_id = $request->appliedPosition;
...
/*        marks value insertion start*/
            $marks=DB::table('pr_exam')
                ->where('status', 1)
                ->get();
            foreach ($marks as $mark)
            {
                if($sscRes==5){
                    $form->sscPoint=$mark->aplus;
                }
                elseif ($sscRes>5 && $sscRes<=4)
                {
                    $form->sscPoint=$mark->a;
                }
                elseif ($sscRes>4 && $sscRes<=3.50)
                {
                    $form->sscPoint=$mark->aminus;
                }
                elseif ($sscRes>4 && $sscRes<=3.50)
                {
                    $form->sscPoint=$mark->aminus;
                }
                elseif ($sscRes>3.50 && $sscRes<=3.00)
                {
                    $form->sscPoint=$mark->b;
                }
                elseif ($sscRes>3.00 && $sscRes<=2.00)
                {
                    $form->sscPoint=$mark->c;
                }
            }

/*        marks value insertion end*/


        $form->user_id = $request->user()->id;
        $form->photo = session('photo');
...
/*        if ($request->NID!="") {
        ...
        $form->save();
            ...
            //$request->session()->forget('doc22ument_ids');
            $request->session()->flash('status', 'This NID Already Exit!');
            redirect ('/application-form');exit();
